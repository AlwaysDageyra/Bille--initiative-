...
function RoleBadge({ role }) {
  ...
  return <span className="inline-flex items-center rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-600">{label}</span>;
}
...
export default function AdminUsers() {
  ...
  const [form, setForm] = useState({ username: "", password: "", role: "submitter", department_id: "" });
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      setForm({ username: "", password: "", role: "submitter", department_id: "" });
      load();
  ...
  const handleSaveEdit = async (id) => {
    setBusy(true);
    try {
      const payload = { role: editRole, department_id: editRole === "dept_manager" ? Number(editDept) : null };
  ...
  return (
    <div>
      <div className="mb-7">
        <h1 className="text-2xl font-extrabold tracking-tight text-ink-900">Users</h1>
        <p className="mt-1 text-sm text-slate-500">Create and manage staff accounts and their roles.</p>
      </div>

      <Card className="mb-8 p-6">
        <h2 className="mb-3 flex items-center gap-2 font-bold text-ink-900"><UserPlus size={16} /> Create User</h2>
            ...
            <tbody>
              {users.map((u) => (
                <tr key={u.id} className="border-b border-slate-50 last:border-0">
                  {editingId === u.id ? (
                    <>
                      <td className="px-5 py-3.5 font-medium text-ink-900">{u.username}</td>
                      <td className="px-5 py-3.5">
                        <select value={editRole} onChange={(e) => setEditRole(e.target.value)} className="rounded-lg border border-slate-300 px-2 py-1.5 text-sm">
                          {ROLE_OPTIONS.map((r) => <option key={r.value} value={r.value}>{r.label}</option>)}
                        </select>
                      </td>
                      <td className="px-5 py-3.5">
                        {editRole === "dept_manager" ? (
                          <select value={editDept} onChange={(e) => setEditDept(e.target.value)} className="rounded-lg border border-slate-300 px-2 py-1.5 text-sm">
                            <option value="">-- select --</option>
                            {departments.map((d) => <option key={d.id} value={d.id}>{d.name}</option>)}
                          </select>
                        ) : "—"}
                    ...
                    <>
                      ...
                      <td className="px-5 py-3.5 text-slate-600">{u.department_name || "—"}</td>
                      <td className="px-5 py-3.5">
